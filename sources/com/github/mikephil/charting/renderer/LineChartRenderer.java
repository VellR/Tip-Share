...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public void drawCubic(Canvas c, ILineDataSet dataSet) {
        ...
        Entry entryFrom = dataSet.getEntryForXIndex(this.mMinX < 0 ? 0 : this.mMinX, Rounding.DOWN);
        Entry entryTo = dataSet.getEntryForXIndex(this.mMaxX, Rounding.UP);
    ...
    public void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            if (dataSet.isVisible() && dataSet.isDrawCirclesEnabled() && dataSet.getEntryCount() != 0) {
                ...
                int count = (int) Math.ceil((double) ((((float) (Math.min(Math.max(minx + 2, dataSet.getEntryIndex(entryTo) + 1), entryCount) - minx)) * phaseX) + ((float) minx)));
