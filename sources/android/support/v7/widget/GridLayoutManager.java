...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (this.mOrientation == 1) {
                measureChildWithDecorationsAndMargin(view2, spec, mainSpec, lp.height == -1, false);
            } else {
                measureChildWithDecorationsAndMargin(view2, mainSpec, spec, lp.width == -1, false);
            }
            int size = this.mOrientationHelper.getDecoratedMeasurement(view2);
            if (size > maxSize) {
                maxSize = size;
            }
            float otherSize = (1.0f * ((float) this.mOrientationHelper.getDecoratedMeasurementInOther(view2))) / ((float) lp.mSpanSize);
        ...
        for (int i4 = 0; i4 < count; i4++) {
            ...
            layoutDecorated(view5, left + params.leftMargin, top + params.topMargin, right - params.rightMargin, bottom - params.bottomMargin);
